...
- \[1110011<ins>0</ins>, 1010010<ins>1</ins>, 0000000<ins>0</ins>], \[1110011<ins>0</ins>, 1010010<ins>0</ins>, 0000000<ins>0</ins>], \[1110011<ins>0</ins>, 1010010<ins>1</ins>, ...
or in decimal:
...
---
The LSB steganography here was implemented using a custom python script.
The Python Imaging Library (**PIL**) library allows image manipulation and processing via a python script.
...
def lsbEncode(b, img):
    ...
    for x in range(0, width):
        for y in range(0, height):
            ...
            # result in the form [194, 190, 179]
						
						# Get each RGB value
            for (i, val) in enumerate(pixel):
                # i = 0, val = 0-255
								
								# Current value of R/G/B
                cval = list(f'{val:08b}') # in the form 00110000
								
								# Check input message is not empty, b = '1001000110...'
                if b: 
										# Set last bit of current value
                    cval[7] = b[0] 
										# Pop first item from input msg
                    b = b[1:] 
                else:
										# If input message is empty
										# Set last bit of current value to 0
										# This will be applied to every following pixel
                    cval[7] = '0'
										
								# Convert current value from base 2 to a decimal number
                pixel[i] = int(''.join(cval), 2)
						
						# Update the image with the new encoded pixel
